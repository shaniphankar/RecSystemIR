...
def spearman_correlation_coefficient(test_data, collab_matrix,orig_matrix):
	...
	return(rho)
	#print(dict2)
def spearman_alt(test_data, collab_matrix,orig_matrix):
	bigSum=0
	countUser=0
	for randUser in range(0,6040):
		countUser+=1
		rankOrig={}
		rankNew={}
		topOrig=np.argsort(orig_matrix[:,randUser])
		topNew=np.argsort(collab_matrix[:,randUser])
		for x in range(3952):
			# print(x)
			rankOrig.update({topOrig[x]:x+1})
			rankNew.update({topNew[x]:x+1})
		sum=0
		for x in range(3952):
			sum+=math.pow(rankNew[x]-rankOrig[x],2)
		rho=1-6*(sum/(3952*(math.pow(3952,2)-1)))
		bigSum+=rho
		print(rho)
	print(bigSum/countUser)
...
def precision_topk_alt(test_data, collab_matrix,orig_matrix,k,threshold):
	''' Definition of the TopK precision, where K is a variable.
    It tells us about the percentage of relevant documents in the top results returned by the system.
    test_data: The data used for testing (30% of the dataset)
    collab_matrix: The matrix of predicted values generated by using a certain approach
	...
	count=0
	for randUser in range(0,6040):
		topKOrig=set(np.flip(np.argsort(orig_matrix[:,randUser]))[:k])
		topKNew=set(np.flip(np.argsort(collab_matrix[:,randUser]))[:k])
		# print(set(topKOrig))
		# print(set(topKNew))
		count+=(len(set(topKOrig).intersection(set(topKNew)))/len(set(topKOrig).union(set(topKNew))))
	print(count/6040)
	# store = sorted(store,key=lambda x: x[0],reverse=True)[:k]
	# count = 0
	# for i in store:
...
def main():
	...
	# print(rho)
	rho = spearman_alt(test_data,collab_matrix,mID_uID_rating)
	# print(rho)
	# pk = precision_topk_alt(test_data,collab_matrix,mID_uID_rating,10,(np.amin(collab_matrix)+np.amax(collab_matrix))/2)
	# print(pk)
	# pk = precision_topk_alt(test_data,collab_matrix,mID_uID_rating,30,2.5)
	# print(pk)
