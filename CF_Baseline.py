...
def find_cosine_similarity(i,j,collab_matrix):
	sum1 = 0
	sum2 = 0
	cosine_score = 0
	for k in range(0,number_of_users):
		cosine_score += collab_matrix[k][i] * collab_matrix[k][j]
	for k in range(0,number_of_users):
		sum1 += math.pow(collab_matrix[k][i],2)
		sum2 += math.pow(collab_matrix[k][j],2)
	if sum1 == 0 or sum2 == 0:
		cosine_score = 0
	else:
		sum1 = math.sqrt(sum1)
...
def get_prediction(top_k,collab_matrix,averageItem,averageUser,overall_average):
	for i in range(0,number_of_items):
		for j in range(0,number_of_users):
			if collab_matrix[j][i] != 0:
				collab_matrix[j][i] -= averageItem[i] + overall_average
	...
	for i in range(0,number_of_items):
		...
		for j in range(0, number_of_items):
			if i != j:
				cosine_score = find_cosine_similarity(i,j,collab_matrix)
			...
			for k in range(0,number_of_users):
				if collab_matrix[k][i]==0:
					...
					for j in range(0,len(cosine_similarity_scores)):
						...
						weighed_sum += weight*(data[k][item] - baseline_other) 
...
def main():
	averageItem,averageUser,overall_average = findAverage(data)
	collab_matrix = extract_data(training_data,data)
